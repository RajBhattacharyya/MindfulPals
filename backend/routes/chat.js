...
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API); // Replace with your actual API key
...
router.post("/messages", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ sent: false, text: response }); // Mark as received
  } catch (error) {
    console.error(error);
